...
RETRAINED_LABELS_TXT_FILE_LOC = os.getcwd() + "/app/ai2/data/retrained_labels.txt"
RETRAINED_GRAPH_PB_FILE_LOC = os.getcwd() + "/app/ai2/data/retrained_graph.pb"

#TEST_IMAGES_DIR = os.getcwd() + "/app/ai2/images/images_to_test"
...
class Test:
    def __init__(self, params):
        global TEST_IMAGES_DIR, TEST_FILE_NAME
        TEST_IMAGES_DIR = params["test_images_dir"]
        TEST_FILE_NAME = params["file_name"]
        print('__init__ test params = ', params)
    ...
    def main(self):
        ...
        if not os.path.isdir(TEST_IMAGES_DIR):
            print(
                "the test image directory does not seem to be a valid directory, check file / directory paths")
            return
        # end if
        result = []
        with tf.Session() as sess:
            ...
            #for fileName in os.listdir(TEST_IMAGES_DIR):
                ...
                #if not (TEST_FILE_NAME.lower().endswith(".jpg") or TEST_FILE_NAME.lower().endswith(".jpeg")):
                #continue
                # end if
                ...
                imageFileWithPath = os.path.join(TEST_IMAGES_DIR, TEST_FILE_NAME)
                ...
                #    print("unable to open " + TEST_FILE_NAME + " as an OpenCV image")
                #    continue
                #end if
                ...
                for prediction in sortedPredictions:
                    ...
                    if onMostLikelyPrediction:
                        ...
                        msg = "the object appears to be a " + strClassification + ", " + "{0:.2f}".format(scoreAsAPercent) + "% confidence"
                        print(msg)
                        result.append(msg)
                        # write the result on the image
                        #writeResultOnImage(
                        #    openCVImage, strClassification + ", " + "{0:.2f}".format(scoreAsAPercent) + "% confidence")
                    ...
                    confi = strClassification + " (" + "{0:.5f}".format(confidence) + ")"
                    print(confi)
                    result.append(confi)
                # end for
...
#         resultText, fontFace, fontScale, fontThickness)
...
#     cv2.putText(openCVImage, resultText, (lowerLeftTextOriginX,
#                                           lowerLeftTextOriginY), fontFace, fontScale, SCALAR_BLUE, fontThickness)
# # end function
