...
AI2_IMAGES_DIR = os.getcwd() + '/app/ai2/images'
AI2_DATA_DIR = os.getcwd() + '/app/ai2/data'
# path to folders of labeled images
# TRAINING_IMAGES_DIR = AI2_DIR + '/training_images'

# TEST_IMAGES_DIR = AI2_DIR + "/test_images/"

# where to save the trained graph
OUTPUT_GRAPH = AI2_DATA_DIR + '/' + 'retrained_graph.pb'

# where to save the intermediate graphs
INTERMEDIATE_OUTPUT_GRAPHS_DIR = AI2_DATA_DIR + '/intermediate_graph'

# how many steps to store intermediate graph, if "0" then will not store
INTERMEDIATE_STORE_FREQUENCY = 0

# where to save the trained graph's labels
OUTPUT_LABELS = AI2_DATA_DIR + '/' + 'retrained_labels.txt'

# where to save summary logs for TensorBoard
TENSORBOARD_DIR = AI2_DATA_DIR + '/' + 'tensorboard_logs'
...
MODEL_DIR = AI2_DATA_DIR + "/" + "model"

# Path to cache bottleneck layer values as files
#BOTTLENECK_DIR = AI2_DIR + '/' + 'bottleneck_data'
...
class Retrain:
    def __init__(self, params):
        global title, TRAINING_IMAGES_DIR, BOTTLENECK_DIR
        title = params["title"]
        TRAINING_IMAGES_DIR = params['image_dir']
        BOTTLENECK_DIR = AI2_DATA_DIR + '/' + 'bottleneck_data/' + title
    def retrain(self):
        print('[retrain] starting program . . . TRAINING_IMAGES_DIR=', TRAINING_IMAGES_DIR)
        ...
        image_lists = create_image_lists(
            TRAINING_IMAGES_DIR, TESTING_PERCENTAGE, VALIDATION_PERCENTAGE)
        class_count = len(image_lists.keys())
        if class_count == 0:
            tf.logging.error(
                'No valid folders of images found at ' + TRAINING_IMAGES_DIR)
            return -1
        # end if
        if class_count == 1:
        ...
        with tf.Session(graph=graph) as sess:
            ...
            train_writer = tf.summary.FileWriter(
                TENSORBOARD_DIR + '/train', sess.graph)
            validation_writer = tf.summary.FileWriter(
                TENSORBOARD_DIR + '/validation')
            ...
            for i in range(HOW_MANY_TRAINING_STEPS):
                ...
                if (intermediate_frequency > 0 and (i % intermediate_frequency == 0) and i > 0):
                    intermediate_file_name = (
                        INTERMEDIATE_OUTPUT_GRAPHS_DIR + 'intermediate_' + str(i) + '.pb')
                    tf.logging.info(
            ...
            if PRINT_MISCLASSIFIED_TEST_IMAGES:
                ...
                for i, test_filename in enumerate(test_filenames):
                    if predictions[i] != test_ground_truth[i]:
                        tf.logging.info('%70s  %s' % (test_filename, list(
...
def checkIfNecessaryPathsAndFilesExist():
    ...
    if not os.path.exists(TRAINING_IMAGES_DIR):
        ...
        print('ERROR: TRAINING_IMAGES_DIR "' +
              TRAINING_IMAGES_DIR + '" does not seem to exist')
    ...
    for dirName in os.listdir(TRAINING_IMAGES_DIR):
        currentTrainingImagesSubDir = os.path.join(
            TRAINING_IMAGES_DIR, dirName)
    ...
    for trainingSubDir in trainingSubDirs:
        if trainingSubDir.numImages < MIN_NUM_IMAGES_SUGGESTED_FOR_TRAINING:
            print("WARNING: there are less than the suggested " +
                  str(MIN_NUM_IMAGES_SUGGESTED_FOR_TRAINING) + " images in " + trainingSubDir.loc)
            print(
                "More images should be added to this directory for acceptable training results")
    ...
    #           str(MIN_NUM_IMAGES_REQUIRED_FOR_TESTING) + " images in " + TEST_IMAGES_DIR)
    #     print("Did you break out some test images?")
    #     return False
    # # end if
...
def makeDirIfDoesNotExist(dir_name):
    ...
    if not os.path.exists(dir_name):
        print('[makeDirIfDoesNotExist] Creating directory = ', dir_name)
...
def create_model_info(architecture):
    ...
    if architecture == 'inception_v3':
        ...
        if (version_string != '1.0' and version_string != '0.75' and version_string != '0.50' and version_string != '0.25'):
            tf.logging.error(
                """"The Mobilenet version should be '1.0', '0.75', '0.50', or '0.25', but found '%s' for architecture '%s'""", version_string, architecture)
            return None
        # end if
        size_string = parts[2]
        ...
        if is_quantized:
            ...
            model_dir_name = ('mobilenet_v1_' + version_string +
                              '_' + size_string + '_quantized_frozen')
...
def downloadModelIfNotAlreadyPresent(data_url):
    ...
    if not os.path.exists(filepath):
        ...
        tf.logging.info('Successfully downloaded ' + str(filename) +
                        ', statinfo.st_size = ' + str(statinfo.st_size) + ' bytes')
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:
        ...
        if len(file_list) < 20:
            ...
            tf.logging.warning('WARNING: Folder {} has more than {} images. Some images will never be selected.'.format(
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir, category, bottleneck_dir, jpeg_data_tensor,
    ...
    try:
        ...
        bottleneckValues = [float(individualString)
                            for individualString in bottleneckBigString.split(',')]
    except ValueError:
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor, bottleneck_tensor_size, quantize_layer):
    ...
    with tf.name_scope('input'):
        bottleneck_input = tf.placeholder_with_default(bottleneck_tensor, shape=[
    ...
    with tf.name_scope(layer_name):
        ...
        with tf.name_scope('Wx_plus_b'):
            if quantize_layer:
                logits = tf.matmul(
                    bottleneck_input, quantized_layer_weights) + quantized_layer_biases
