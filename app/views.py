...
from app.ai2.retrain import Retrain
from app.ai2.test import Test
...
UPLOAD_FOLDER = 'app/ai2/images/upload_folder'
UPLOAD_FOLDER_ZIP = 'app/ai2/images/upload_folder_zip'
DATASET_UPLOAD_FOLDER = 'app/ai2/images/images_to_train/'
UPLOAD_FOLDER2 = 'app/ai2/images/images_to_test'
ALLOWED_EXTENSIONS = set(['zip', 'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', '7z'])
...
def index():
    ...
    models.insert(0, {'id': 0, 'title': 'Pre trained',
                      'folder_name': 'default', 'checked': True})
...
def startTraining(image_dir, title, id):
    try:
        ...
        print('Initiating learning tu2', params)
...
def createDirIfDoesNotExist(dir_name):
    """
    ...
    if not os.path.exists(dir_name):
        print('[createDirIfDoesNotExist] Creating directory = ', dir_name)
...
def clean_zip_for_learning(data):
    ...
    createDirIfDoesNotExist(trained_model_dir)
    # foreach labels
    for zip in zips:
...
def api_evaluate():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            dir_path = os.path.join(UPLOAD_FOLDER2, model_dir_name)
            createDirIfDoesNotExist(dir_path)
            dir_with_file_path = os.path.join(dir_path, filename)
            print('dir_with_file_path=', dir_with_file_path)
            file.save(dir_with_file_path)
            print('starting test')
            params = {'file_name': filename, 'test_images_dir': dir_path}
            tester = Test(params)
            result = tester.main()
            ...
            return json.dumps(['sample_test/' + dir_with_file_path, result])
    return 'Failure'
